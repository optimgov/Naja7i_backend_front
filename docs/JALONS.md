...
| 1.3 | **3B — la consommation** (M-008) | Débit idempotent `(user, tentative, item)` au premier service ; **une seule enveloppe effective** ; coût annoncé avant composition ; bornes anti-aspiration (Q-22) ; S-09/S-10 | **Livré** — `2e6ae28` · **dormant** : rien n'est posé sur une base durable | 1.2 |
...
| 2.3 | **Q2 service — l'interface experts** | File de qualification, saisie corrigés/justifications (jamais pré-remplie), permission dédiée (Q-10), échelle de difficulté en données éditables (Q-09, DET-86), `hors_nomenclature` (DET-16), retranscription (C-A), invariant doublons (C-B), signalement éditorial structuré | **Livré** — `7f95aac` · Q-09 et Q-10 appliquées comme valeurs de départ, révocables à l'écran | Q-09/Q-10 |
