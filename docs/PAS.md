...
| DET-101 | Le slug d'une spécialité porte son parcours (`langue-francaise-secondaire`), et l'unicité `(exam_family_id, slug)` que `000220` avait retirée revient : la route publique adresse par `(famille, slug)`, un couple qui n'identifiait plus rien depuis que les spécialités pendent du parcours — sur la préproduction elle rendait le primaire en liste d'attente, et la seule spécialité OUVERTE du pilote n'était atteignable par aucune URL | `689b66b` |
| M-v2-SE | L'arbre avant le nœud : `prepared_questions.exam_id` pour dire « on sait l'épreuve, pas encore le domaine », avec un trigger qui refuse un nœud d'une autre épreuve ; l'import du corpus QCM en zone de préparation, rangé par arbre et JAMAIS qualifié — le pré-classement du 15 août reste dans `provisional`, où il se lit comme l'aide de script qu'il est ; et le retrait des brouillons d'un ancien import, qui refuse en bloc dès qu'une question a servi | `b41094a` |
