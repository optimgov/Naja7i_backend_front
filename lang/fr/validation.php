...
|--------------------------------------------------------------------------
|
| PAS-3.1. Ce fichier était absent : Laravel retombait alors sur la clé de
| traduction elle-même et le candidat lisait « validation.min.string » sous le
| champ mot de passe. La recette FRONT-1 l'a constaté en point 10.
|
| Le fichier est traduit EN ENTIER, pas seulement pour les règles employées
| aujourd'hui. Une règle ajoutée demain dans un FormRequest ne doit pas pouvoir
| réintroduire une clé brute à l'écran : c'est une régression silencieuse, que
| rien ne signale tant qu'un utilisateur ne la rencontre pas.
|
| Les messages s'adressent à un candidat, pas à un développeur : on nomme le
| champ en clair grâce à `attributes`, et on reformule dans `custom` les
| tournures que le gabarit générique rendrait maladroites.
|
*/
...
return [
    ...
    'password' => [
        ...
        // Message exposé le plus souvent après `min` : la politique impose le
        // contrôle anti-fuite (ADR-0007). Il doit expliquer sans culpabiliser
        // — le candidat n'a rien fait de mal, son mot de passe circule.
        'uncompromised' => 'Ce mot de passe figure dans une fuite de données connue. Choisissez-en un autre.',
    ],
    ...
    |--------------------------------------------------------------------------
    |
    | Le gabarit générique « Le champ :attribute … » devient maladroit sur
    | quelques combinaisons — « Le champ adresse e-mail doit être une adresse
    | e-mail valide. » Ces cas-là sont réécrits ici, en s'adressant au candidat.
    |
    | Les refus de CGU et de politique de confidentialité ne figurent pas ici :
    | RegisterRequest::messages() les couvre déjà avec `auth.terms_required` et
    | `auth.privacy_required`, plus explicites parce qu'ils rappellent l'enjeu.
    |
    */
    ...
    'custom' => [
        'email' => [
            ...
            'max' => "L'adresse e-mail ne doit pas dépasser :max caractères.",
        ],
        'password' => [
            'required' => 'Le mot de passe est obligatoire.',
            'confirmed' => 'Les deux mots de passe saisis ne sont pas identiques.',
            'min' => 'Le mot de passe doit contenir au moins :min caractères.',
            'max' => 'Le mot de passe ne doit pas dépasser :max caractères.',
        ],
        'locale' => [
            'in' => 'La langue choisie doit être le français ou l\'arabe.',
        ],
    ],
    ...
    |--------------------------------------------------------------------------
    |
    | Sans cette table, les messages nomment le champ par sa clé technique —
    | « Le champ terms_accepted doit être accepté. » Le candidat n'a aucune
    | raison de connaître nos noms de colonnes.
    |
    */
    ...
    'attributes' => [
        ...
        'terms_accepted' => 'acceptation des conditions générales',
        'privacy_notice_acknowledged' => 'prise de connaissance de la politique de confidentialité',
        'marketing_granted' => 'consentement aux rappels',
        'locale' => 'langue',
        'token' => 'jeton',
        'remember' => 'rester connecté',
        'granted' => 'consentement',
